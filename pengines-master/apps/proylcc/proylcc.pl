...
% 	�nicamente se usa en el conteo de puntos, ya que se tiene la certeza de que el tablero no cambiar�.
% 
...
%
% 	RBoard es la configuraci�n resultante de reflejar la movida del jugador Player
% 	en la posici�n Pos a partir de la configuraci�n Board.
% 	Luego de colocar una ficha de Player en Pos, se busca si alg�n adyacente ha sido 
% 	encerrado. De ser as�, son eliminados, devolviendo el tablero en RBoard.
% 	Luego se chequea si la posici�n donde se ha colocado la ficha no es sucidio.
% 
% 
...
%     Si no fue as�, recursivamente visita sus vecinos para chequear si esta encerrado.
%   
...
%   Dado que el metodo usado para marcar visitados (checked/2) es din�mico, puede cambiar
%   din�micamente, produciendo as� soluciones alternativas no deseadas.
%
...
%   En caso de que el Index este fuera del tablero, devuelvo la lista vac�a.
%
...
%   Si no esta encerrado, marca los chequeados como no encerrados, ya que el tablero no cambiar�.
%   
...
%	El siguiente ser� el vecino derecho, 
%	o bien el primero de la fila siguiente, si el index dado es de la �ltima columna.
%
...
%	Termina cuando no hay m�s marcados como chequeados.
%
